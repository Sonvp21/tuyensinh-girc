...
DataTable.ext.renderer.pagingButton.tailwind = function (settings, buttonType, content, active, disabled) {
	...
	if (active) {
		btnClasses.push('bg-blue-600 text-white shadow-md rounded-sm');
	}
	...
	var a = $('<button>', {
		'class': 'page-link pb-1 w-6 h-6 border border-gray-300 rounded-sm flex items-center justify-center hover:bg-blue-100 hover:text-blue-600 transition-all duration-200 ease-in-out',
		role: 'link',
		type: 'button'
	})
		.html(content)
