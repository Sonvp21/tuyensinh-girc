<x-admin-layout>
    ...
    <div class="flex-grow w-full md:p-5 p-1">
        <div class="mb-5">
            <h1 class="text-2xl font-semibold text-gray-800">Thông báo</h1>
            <nav class="mt-2">
            ...
            <div class="card-body p-0">
                <div class="text-gray-800 text-3xl font-semibold leading-tight flex pb-1">
                    <div class="flex ml-auto">
                        <a class="btn btn-outline btn-accent !min-h-10 h-10 w-fit" href="{{ route('admin.notifies.create') }}">
                        ...
                        <table id="example" class="table min-w-full border border-gray-300 mt-3">
                            ...
                            <tbody>
                                @foreach ($notifies as $index => $notify)
                                    <tr>
                                        <th>{{ $loop->index + 1 }}</th>
                                        <td class="text-left font-semibold">{{ Str::limit($notify->title, 80) }}</td>
                                        <td>{{ $notify->updatedAtVi }}</td>
                                        <td>
                                            <label class="label cursor-pointer place-content-center">
                                                <input type="checkbox" class="!toggle !toggle-accent"
                                                    @if ($notify->is_visible) checked="checked" @endif
                                                    onclick="toggleVisibility('{{ $notify->id }}')" />
                                            </label>
                                        </td>

                                        <td class="!flex justify-center m-2">
                                            <a href="{{ route('admin.notifies.edit', $notify) }}" class="mr-2"
                                                type="button"><i
                                                    class="fa fa-edit text-yellow-400 hover:text-yellow-500"></i></a>

                                            <!-- Nút mở modal -->
                                            <button class="p-0"
                                                onclick="document.getElementById('confirm-modal-{{ $notify->id }}').showModal()">
                                                <i
                                                    class="fad fa-trash-alt text-red-400 hover:text-red-500 cursor-pointer"></i>
                                            </button>
                                            <!-- Modal xác nhận xóa -->
                                            <dialog id="confirm-modal-{{ $notify->id }}" class="modal">
                                                <style>
                                                    .modal-box {
                                                        max-width: 25rem;
                                                        position: relative;
                                                ...
                                                <div class="modal-box">
                                                    ...
                                                    <p class="py-4 text-base text-center">Bạn có chắc muốn xoá thông báo này?</p>

                                                    <div class="modal-action justify-center">
                                                        <!-- Nút hủy -->
                                                        <button class="btn bg-gray-500 text-white px-6"
                                                            onclick="document.getElementById('confirm-modal-{{ $notify->id }}').close()">Hủy</button>

                                                        <!-- Nút xác nhận, gọi form xóa -->
                                                        <button class="btn btn-error text-white px-6"
                                                            onclick="document.getElementById('delete-form-{{ $notify->id }}').submit()">Xoá</button>
                                                    </div>
                                                </div>
                                            </dialog>
                                            <!-- Form xóa ẩn -->
                                            <form id="delete-form-{{ $notify->id }}"
                                                action="{{ route('admin.notifies.destroy', $notify) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </td>
    ...
    @pushOnce('bottom_scripts')
        ...
        <script>
            function toggleVisibility(notifyId) {
                fetch(`/admin/notifies/${notifyId}/toggle-visibility`, {
                        method: 'POST',
                        headers: {
                    ...
                    .then(data => {
                        if (data.success) {
                            showToast('Trạng thái hiển thị của thông báo đã được cập nhật.', 'success');
                        } else {
